...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $iconic_star = $_POST["iconic_star"];
    
    // Convert the array to a comma-separated string
    $iconicStarString = implode(',', $iconic_star);
    $id = $conn->real_escape_string($_SESSION["id"]);
    $iconicStarString = $conn->real_escape_string($iconicStarString);

    $sql = "UPDATE person SET  iconic_star = '$iconicStarString' WHERE person_id = '$id'";

    if ($conn->query($sql) === TRUE) {
       header("Location:dream-country.php");
    } else {
...
<meta charset="UTF-8">
 ...
 <link rel="icon" href="favicon.ico" type="image/x-icon">
    ...
    <style>
        ...
        .background {
            background-image: url('images/iconic-star.jpg');
...
<script>
    function addInputField() {
        ...
        inputField.name = 'iconic_star[]'; // Use array notation for multiple values
